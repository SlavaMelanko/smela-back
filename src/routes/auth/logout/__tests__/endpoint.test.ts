...
describe('Logout Endpoint', () => {
  ...
  describe('POST /auth/logout', () => {
    it('should delete refresh token cookie and return 204 No Content', async () => {
      const res = await post(app, LOGOUT_URL, undefined, {
        Cookie: 'refresh_token=existing-token',
      })
      ...
      // Note: Client also clears access token from memory
    ...
    it('should handle different cookie headers', async () => {
      const cookieScenarios = [
        ...
        { name: 'refresh token cookie', headers: { Cookie: 'refresh_token=jwt-token-value' } },
        { name: 'multiple cookies with refresh', headers: { Cookie: 'refresh_token=jwt-token; other=value' } },
      ]
    ...
    it('should handle multiple consecutive logout calls gracefully', async () => {
      ...
      for (let i = 0; i < numCalls; i++) {
        const res = await post(app, LOGOUT_URL, undefined, {
          Cookie: 'refresh_token=jwt-token-value',
        })
